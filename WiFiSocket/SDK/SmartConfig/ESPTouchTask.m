...
- (BOOL) __execute: (ESPTouchGenerator *)generator
{
    ...
    {
        if (currentTime - lastTime >= [self._parameter getTimeoutTotalCodeMillisecond]/400.0)
        {
            if (DEBUG_ON)
            {
            ...
            // send guide code
            while (!self._isInterrupt && [[NSDate date] timeIntervalSince1970] - currentTime < [self._parameter getTimeoutGuideCodeMillisecond]/400.0)
            {
                ...
                if ([[NSDate date] timeIntervalSince1970] - startTime > [self._parameter getWaitUdpSendingMillisecond]/400.0)
                {
                    break;
                }
            }
